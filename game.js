...
//  game.js — часть 1/3: константы, состояние, кадры
// ============================================================
...
const FIRST_NAMES_M = [
  ...
  'Руслан','Валерий','Станислав','Борис','Геннадий','Аркадий','Леонид','Пётр'
];
const FIRST_NAMES_F = [
  'Анна','Мария','Ольга','Екатерина','Татьяна','Светлана','Ирина',
  'Наталья','Юлия','Виктория','Елена','Дарья','Алиса','Полина','Ксения',
  'Вероника','Алина','Кристина','Марина','Оксана','Людмила','Валентина'
];
const LAST_NAMES = [
...
const CRIMES = [
  ...
  'Нападение на полицейский участок','Захват автобуса'
];
const STREETS = [
  'ул. Ленина','пр. Мира','ул. Гагарина','ул. Советская','пр. Победы',
  'ул. Кирова','ул. Пушкина','ул. Чехова','ул. Гоголя','ул. Тверская',
  'ул. Арбат','пр. Ленинградский','ул. Садовая','ул. Лесная','ул. Новая',
  'ул. Центральная','ул. Школьная','ул. Заводская'
];
const JOBS = [
...
const Game = {
  ...
  init() {
    ...
    this.log('Игра запущена. Выберите ведомство.', 'info');
  },
  ...
  // ----------------------------------------
  //  ГЕНЕРАЦИЯ И ОТОБРАЖЕНИЕ КАНДИДАТА
  // ----------------------------------------
  generateCandidate() {
      ...
      <div class="docs-grid">
        <div class="doc-card">
          ...
          <div class="doc-row"><span>Судимости:</span>
            <b class="${crim ? 'doc-bad' : 'doc-good'}">${crim ? '⚠ ЕСТЬ' : 'Нет'}</b></div>
          <div class="doc-row"><span>Долги:</span>
            <b class="${deb ? 'doc-warn' : 'doc-good'}">${deb ? '⚠ Есть' : 'Нет'}</b></div>
        </div>

        <div class="doc-card">
          <div class="doc-title">🩺 Медкарта</div>
          <div class="doc-row"><span>Статус:</span>
            <b class="${med.healthy ? 'doc-good' : 'doc-bad'}">${med.healthy ? 'Здоров' : med.issue}</b></div>
          <div class="doc-row"><span>Психика:</span>
            <b class="${med.psych > 80 ? 'doc-good' : med.psych > 60 ? 'doc-warn' : 'doc-bad'}">${med.psych}/100</b></div>
        </div>

        <div class="doc-card">
          <div class="doc-title">🎖️ Военный билет</div>
          <div class="doc-row"><span>Служба:</span>
            <b class="${mil.served ? 'doc-good' : 'doc-bad'}">${mil.served ? 'Да' : 'Нет'}</b></div>
          <div class="doc-row"><span>Категория:</span><b>${mil.category}</b></div>
          <div class="doc-row"><span>Горячие точки:</span>
            <b class="${mil.hotSpots ? 'doc-warn' : ''}">${mil.hotSpots ? 'Да (' + mil.hotspot + ')' : 'Нет'}</b></div>
        </div>
...
// ============================================================
//  ПОЛКОВНИК — Симулятор ведомства
//  game2.js — часть 2/3: сотрудники, больничный, группы
// ============================================================
    ...
    el.innerHTML = State.groups.map(g => {
      ...
      return `
        <div class="group-card ${woundedCount > 0 ? 'wounded' : ''}">
          ...
          <button class="btn small danger" style="margin-top:8px" onclick="Game.deleteGroup(${g.id})">Расформировать</button>
        </div>
      `;
    }).join('');
  },
...
// ============================================================
//  ПОЛКОВНИК — Симулятор ведомства
//  game3.js — часть 3/3: операции, розыск, сейвы, UI
// ============================================================
...
Object.assign(Game, {
  ...
  sendGroup(eventId) {
    ...
    const fighters = g.members
      .map(id => this.findStaff(id))
    ...
    if (chance(successChance)) {
      ...
      this.log(`✅ Группа «${g.name}» ликвидировала: ${e.title}. +20000₽, +3 реп.`, 'success');
  ...
  renderWanted() {
    ...
    el.innerHTML = State.wanted.map(w => `
      <div class="wanted-card">
        ...
        <button class="btn small" style="margin-top:8px" onclick="Game.catchWanted(${w.id})">🚔 Задержать</button>
      </div>
    `).join('');
  },
  ...
  saveGame() {
    try {
      ...
      this.toast('💾 Игра сохранена', 'success');
      this.log('💾 Сохранение создано', 'info');
    } catch (err) {
      this.toast('Ошибка сохранения: ' + err.message, 'danger');
    }
  },
  ...
  resetGame() {
    if (!confirm('Точно сбросить ВСЁ? Сохранение будет удалено.')) return;
